...
: QDialog(parent, name, modal, f)
{
	...
	//-------------------------------------------------------------------------
	QWidget * pTabIncidents = new QWidget(this);
	QSplitter * splitterIncidents = new QSplitter(Qt::Vertical, pTabIncidents);
	QWidget * pIncidentsBox = new QWidget(splitterIncidents);
	QWidget * pIncidentsButtonBox = new QWidget(pIncidentsBox);
	QWidget * pIncidentTypeBox = new QWidget(splitterIncidents);
	QWidget * pIncidentNameBox = new QWidget(pIncidentTypeBox);
	QWidget * pIncidentButtons = new QWidget(pTabIncidents);
	QVBoxLayout * pTabIncidentsLayout = new QVBoxLayout(pTabIncidents, 8, 8);
	QHBoxLayout * pIncidentsBoxLayout = new QHBoxLayout(pIncidentsBox, 0, 8);
	QVBoxLayout * pIncidentsButtonBoxLayout = new QVBoxLayout(pIncidentsButtonBox, 0, 8);
	QVBoxLayout * pIncidentTypeBoxLayout = new QVBoxLayout(pIncidentTypeBox, 0, 8);
	QHBoxLayout * pIncidentNameBoxLayout = new QHBoxLayout(pIncidentNameBox, 0, 8);
	QHBoxLayout * pIncidentButtonsLayout = new QHBoxLayout(pIncidentButtons, 0, 8);
	//----------------------added by Cong--------------------------------------
	...
	//------------------------------------------------------------------------
	IncidentModel::GetParams(m_mapModelParams[INCIDENTMODEL_NAME]);
	m_vecModelTypes.push_back(INCIDENTMODEL_NAME);
	IncidentListVisual::GetParams(m_mapModelParams[INCIDENTLISTVISUAL_NAME]);
	m_vecModelTypes.push_back(INCIDENTLISTVISUAL_NAME);
	VehicleModel::GetParams(m_mapModelParams[VEHICLEMODEL_NAME]);
	m_vecModelTypes.push_back(VEHICLEMODEL_NAME);
	AuthorityRoleModel::GetParams(m_mapModelParams[AUTHORITYROLEMODEL_NAME]);
	m_vecModelTypes.push_back(AUTHORITYROLEMODEL_NAME);
	AttackerRoleModel::GetParams(m_mapModelParams[ATTACKERROLEMODEL_NAME]);
	m_vecModelTypes.push_back(ATTACKERROLEMODEL_NAME);
	DefaultRoleModel::GetParams(m_mapModelParams[DEFAULTROLEMODEL_NAME]);
	m_vecModelTypes.push_back(DEFAULTROLEMODEL_NAME);
	//--------------------------added by Cong--------------------------------
	...
	//-------------------------------------------------------------
	m_listIncidents = new QListBox(pIncidentsBox);
	m_buttonIncidentAdd = new QPushButton("Add", pIncidentsButtonBox);
	m_pIncidentsAddMenu = new QPopupMenu(this);
	m_buttonIncidentRemove = new QPushButton("Remove", pIncidentsButtonBox);
	m_buttonIncidentDuplicate = new QPushButton("Duplicate", pIncidentsButtonBox);
	m_lblIncidentName = new QLabel("Model Name", pIncidentNameBox);
	m_txtIncidentName = new QLineEdit(pIncidentNameBox);
	m_tableIncidentProperties = new QTable(0, 2, pIncidentTypeBox);
	m_txtIncidentPropertiesHelp = new QTextEdit("", QString::null, splitterIncidents);
	m_buttonIncidentAutoGen = new QPushButton("Auto-Generate", pIncidentButtons);
	m_buttonIncidentRemoveAll = new QPushButton("Remove All", pIncidentButtons);
	connect(m_listIncidents, SIGNAL(highlighted(int)), this, SLOT(slotIncidentsListHighlighted(int)));
	connect(m_listIncidents, SIGNAL(selectionChanged()), this, SLOT(slotIncidentsListSelChanged()));
	connect(m_pIncidentsAddMenu, SIGNAL(activated(int)), this, SLOT(slotIncidentsAdd(int)));
	connect(m_buttonIncidentRemove, SIGNAL(clicked()), this, SLOT(slotIncidentsRemove()));
	connect(m_buttonIncidentDuplicate, SIGNAL(clicked()), this, SLOT(slotIncidentsDuplicate()));
	connect(m_txtIncidentName, SIGNAL(textChanged(const QString& )), this, SLOT(slotIncidentsNameChanged(const QString& )));
	connect(m_tableIncidentProperties, SIGNAL(valueChanged(int, int)), this, SLOT(slotIncidentsPropertiesValueChanged(int, int )));
	connect(m_tableIncidentProperties, SIGNAL(currentChanged(int, int)), this, SLOT(slotIncidentsPropertiesCurrentChanged(int, int )));
	connect(m_buttonIncidentAutoGen, SIGNAL(clicked()), this, SLOT(slotIncidentsAutoGen()));
	connect(m_buttonIncidentRemoveAll, SIGNAL(clicked()), this, SLOT(slotIncidentsRemoveAll()));
	m_listIncidents->setSelectionMode(QListBox::Extended);
	m_listIncidents->clearSelection();
	//-----------------------------added by Cong-------------------
	...
	for (i = 0; i < m_vecModelTypes.size(); i++)
	{
		...
		else if (g_pSimulator->m_ModelMgr.IsModelTypeOf(m_vecModelTypes[i], SIMMODEL_NAME))
			m_pVehiclesAddMenu->insertItem(m_vecModelTypes[i], i); // added by Cong
		...
		else if (g_pSimulator->m_ModelMgr.IsModelTypeOf(m_vecModelTypes[i], INCIDENTMODEL_NAME))
			m_pIncidentsAddMenu->insertItem(m_vecModelTypes[i], i); //added by Cong
		else
...
//	pModelTypeBoxLayout->addWidget(m_txtModelPropertiesHelp, 0);
	...
	//-------------------------------------------------------------------------
	m_buttonIncidentAdd->setPopup(m_pIncidentsAddMenu);
	m_tabsTabs->addTab(pTabIncidents, "Incidents");
	m_tableIncidentProperties->verticalHeader()->hide();
	m_tableIncidentProperties->setLeftMargin(0);
	m_tableIncidentProperties->horizontalHeader()->setClickEnabled(false);
	m_tableIncidentProperties->horizontalHeader()->setMovingEnabled(false);
	m_tableIncidentProperties->horizontalHeader()->setLabel(0, "Parameter");
	m_tableIncidentProperties->horizontalHeader()->setLabel(1, "Value");
	m_tableIncidentProperties->setSorting(false);
	m_tableIncidentProperties->setSelectionMode(QTable::SingleRow);
	m_tableIncidentProperties->setFocusStyle(QTable::FollowStyle);
	m_tableIncidentProperties->setColumnReadOnly(0, true);
	m_tableIncidentProperties->setColumnReadOnly(1, false);
	m_tableIncidentProperties->setColumnStretchable(1, true);
	m_txtIncidentPropertiesHelp->setReadOnly(true);
	m_buttonIncidentRemoveAll->setEnabled(m_listIncidents->count() > 0);
	m_listVehicles->setCurrentItem(m_listIncidents->count() > 0 ? 0 : -1);
	slotIncidentsListHighlighted(m_listIncidents->currentItem());
	slotIncidentsListSelChanged();
	pTabIncidentsLayout->addWidget(splitterIncidents, 1);
	pTabIncidentsLayout->addWidget(pIncidentButtons);
	pIncidentsBoxLayout->addWidget(m_listIncidents, 1);
	pIncidentsBoxLayout->addWidget(pIncidentsButtonBox);
	pIncidentsButtonBoxLayout->addWidget(m_buttonIncidentAdd, 0, Qt::AlignCenter);
	pIncidentsButtonBoxLayout->addWidget(m_buttonIncidentRemove, 0, Qt::AlignCenter);
	pIncidentsButtonBoxLayout->addWidget(m_buttonIncidentDuplicate, 0, Qt::AlignCenter);
	pIncidentsButtonBoxLayout->addStretch(1);
	pIncidentTypeBoxLayout->addWidget(pIncidentNameBox);
	pIncidentTypeBoxLayout->addWidget(m_tableIncidentProperties, 1);
//	pIncidentTypeBoxLayout->addWidget(m_txtIncidentPropertiesHelp, 0);
	pIncidentNameBoxLayout->addWidget(m_lblIncidentName);
	pIncidentNameBoxLayout->addWidget(m_txtIncidentName, 1);
	pIncidentButtonsLayout->addWidget(m_buttonIncidentAutoGen, 0, Qt::AlignCenter);
	pIncidentButtonsLayout->addWidget(m_buttonIncidentRemoveAll, 0, Qt::AlignCenter);
	//-------------------------------added by Cong-----------------------------	
...
//----------------------------------------------------------------------------
void QSimCreateDialog::slotIncidentsListHighlighted(int index)
{
	m_lblIncidentName->setEnabled(index > -1);
	m_txtIncidentName->setEnabled(index > -1);
	disconnect(m_txtIncidentName, SIGNAL(textChanged(const QString& )), this, SLOT(slotIncidentsNameChanged(const QString& )));
	disconnect(m_tableIncidentProperties, SIGNAL(valueChanged(int, int)), this, SLOT(slotIncidentsPropertiesValueChanged(int, int)));
	if (index > -1)
	{
		int iRow;
		std::map<QString, std::map<QString, QString> >::iterator iterModel = m_mapModels.find(m_vecIncidents[index].first);
		std::map<QString, std::map<QString, ModelParameter> >::iterator iterParams = m_mapModelParams.find(m_vecIncidents[index].second);
		std::map<QString, ModelParameter>::iterator iterParam;
		m_txtIncidentName->setText(m_vecIncidents[index].first);
		if (iterModel != m_mapModels.end())
		{
			std::map<QString, QString>::iterator iterValue;
			m_tableIncidentProperties->setNumRows(0);
			m_tableIncidentProperties->setNumRows(iterModel->second.size());
			for (iterValue = iterModel->second.begin(), iRow = 0; iterValue != iterModel->second.end(); ++iterValue, iRow++)
			{
				iterParam = iterParams->second.find(iterValue->first);
				m_tableIncidentProperties->setText(iRow, 0, iterValue->first);
				if (iterParam != iterParams->second.end())
				{
					AddTableItem(m_tableIncidentProperties, iRow, 1, iterValue->second, &iterParam->second);
				}
			}
			m_tableIncidentProperties->sortColumn(0, true, true);
		}
		else
		{
			m_tableIncidentProperties->setNumRows(0);		
	...
	{
		m_txtIncidentName->setText("");
		m_tableIncidentProperties->setNumRows(0);
	}
	slotIncidentsPropertiesCurrentChanged(-1, -1);
	connect(m_tableIncidentProperties, SIGNAL(valueChanged(int, int)), this, SLOT(slotIncidentsPropertiesValueChanged(int, int)));
	connect(m_txtIncidentName, SIGNAL(textChanged(const QString& )), this, SLOT(slotIncidentsNameChanged(const QString& )));
	m_tableIncidentProperties->adjustColumn(0);
}

void QSimCreateDialog::slotIncidentsListSelChanged()
{
	bool bSelection = false;
	unsigned int i;
	for (i = 0; i < m_listIncidents->count(); i++)
	{
		if (m_listIncidents->isSelected(i))
		{
			bSelection = true;
			break;
		}
	}
	m_buttonIncidentRemove->setEnabled(bSelection);
	m_buttonIncidentDuplicate->setEnabled(bSelection);
}

void QSimCreateDialog::slotIncidentsAdd(int id)
{
	if (id > -1)
	{
		int index = m_listIncidents->currentItem() + 1;
		QString strIncidentType = m_vecModelTypes[id], strName = GetNextModelName(strIncidentType);
		std::vector<std::pair<QString, QString> >::iterator iterIndex = m_vecIncidents.begin();
		std::map<QString, std::map<QString, QString> >::iterator iterModel = m_mapModels.insert(std::pair<QString, std::map<QString, QString> >(strName, std::map<QString, QString>())).first;
		std::map<QString, std::map<QString, ModelParameter> >::iterator iterParams = m_mapModelParams.find(strIncidentType);
		std::map<QString, ModelParameter>::iterator iterParam;
		for (iterParam = iterParams->second.begin(); iterParam != iterParams->second.end(); ++iterParam)
		{
			iterModel->second[iterParam->first] = iterParam->second.strValue;
		}
		if (index >= (signed)m_vecIncidents.size())
		{
			iterIndex = m_vecIncidents.end();
		}
		else
		{
			iterIndex += index;
		}
		iterIndex = m_vecIncidents.insert(iterIndex, std::pair<QString, QString>(strName, strIncidentType));
		m_listIncidents->insertItem(QString("[%1] %2").arg(iterIndex->second).arg(iterIndex->first), index);
		m_setModelNames.insert(strName);
		m_buttonIncidentRemoveAll->setEnabled(m_listIncidents->count() > 0);
		m_listIncidents->setCurrentItem(index);
		slotIncidentsListHighlighted(m_listIncidents->currentItem());
	}
}
void QSimCreateDialog::slotIncidentsRemove()
{
	unsigned int index = 0;
	while (index < m_listIncidents->count())
	{
		if (m_listIncidents->isSelected(index))
		{
			std::map<QString, std::map<QString, QString> >::iterator iterValues = m_mapModels.find(m_vecIncidents[index].first);
			std::vector<pair<QString, QString> >::iterator iterName = m_vecIncidents.begin();
			iterName += index;
			m_mapModels.erase(iterValues);
			m_vecIncidents.erase(iterName);
			m_listIncidents->removeItem(index);
		}
		else
		{
			index++;
...
void QSimCreateDialog::slotIncidentsDuplicate()
{
	unsigned int index;
	for (index = 0; index < m_listIncidents->count(); index++)
	{
		if (m_listIncidents->isSelected(index))
		{
			std::vector<std::pair<QString, QString> >::iterator iterName = m_vecIncidents.begin(), iterNextName;
			QString strName = GetNextModelName(iterName->second);
			iterName += index;
			
			iterNextName = m_vecIncidents.insert(m_vecIncidents.end(), std::pair<QString, QString>(strName, iterName->second));
			m_listIncidents->insertItem(QString("[%1] %2").arg(iterNextName->second).arg(iterNextName->first));
			m_setModelNames.insert(strName);
		}
	}
	m_buttonIncidentRemoveAll->setEnabled(m_listIncidents->count() > 0);
...
void QSimCreateDialog::slotIncidentsNameChanged(const QString & strName)
{
	int index = m_listIncidents->currentItem();
	int iCursor = m_txtIncidentName->cursorPosition();
	disconnect(m_listIncidents, SIGNAL(highlighted(int)), this, SLOT(slotIncidentsListHighlighted(int)));
	if (index > -1)
	{
		bool bSelected = m_listIncidents->isSelected(index);
		QString strNameOld = m_vecIncidents[index].first;
		std::vector<std::pair<QString, QString> >::iterator iterName = m_vecIncidents.begin();
		std::map<QString, std::map<QString, QString> >::iterator iterValues = m_mapModels.find(strNameOld);
		...
		if (m_setModelNames.insert(strName).second)
		{
			m_setModelNames.erase(strNameOld);
			m_mapModels.erase(iterValues);
			m_vecIncidents[index].first = strName;
			m_listIncidents->changeItem(QString("[%1] %2 (%3)").arg(m_vecIncidents[index].second).arg(strName), index);
		}
		m_listIncidents->setSelected(index, bSelected);
	}
	connect(m_listIncidents, SIGNAL(highlighted(int)), this, SLOT(slotIncidentsListHighlighted(int)));
	m_txtIncidentName->setCursorPosition(iCursor);
}

void QSimCreateDialog::slotIncidentsPropertiesValueChanged(int row, int col)
{
	int index = m_listIncidents->currentItem();
	disconnect(m_listIncidents, SIGNAL(highlighted(int)), this, SLOT(slotIncidentsListHighlighted(int)));
	if (index > -1 && row > -1 && col == 1)
	{
		bool bSelected = m_listIncidents->isSelected(index);
		QString strParam = m_tableIncidentProperties->text(row, 0);
		QString strValue = m_tableIncidentProperties->text(row, col);
		std::vector<std::pair<QString, QString> >::iterator iterName = m_vecIncidents.begin();
		std::map<QString, std::map<QString, QString> >::iterator iterValues = m_mapModels.find(m_vecIncidents[index].first);
		std::map<QString, ModelParameter>::iterator iterParam = m_mapModelParams[m_vecIncidents[index].second].find(strParam);
		iterName += index;
		if (iterParam == m_mapModelParams[m_vecIncidents[index].second].end() || ValidateTableItem(strValue, &iterParam->second))
		{
			iterValues->second[strParam] = strValue;
		}
		else
		{
			AddTableItem(m_tableIncidentProperties, row, col, iterValues->second[strParam], &iterParam->second);
		}
		m_listIncidents->setSelected(index, bSelected);
	}
	connect(m_listIncidents, SIGNAL(highlighted(int)), this, SLOT(slotIncidentsListHighlighted(int)));
...
void QSimCreateDialog::slotIncidentsPropertiesCurrentChanged(int row, int col)
{
	int index = m_listIncidents->currentItem();
	if (index > -1 && row > -1)
	{
		QString strParam = m_tableIncidentProperties->text(row, 0);
		std::map<QString, ModelParameter>::iterator iterParam = m_mapModelParams[m_vecIncidents[index].second].find(strParam);
		if (iterParam != m_mapModelParams[m_vecIncidents[index].second].end())
			m_txtIncidentPropertiesHelp->setText(iterParam->second.strDesc);
		else
			m_txtIncidentPropertiesHelp->setText("");
	}
	else
		m_txtIncidentPropertiesHelp->setText("");
}

void QSimCreateDialog::slotIncidentsAutoGen()
{
	// to be added	
}

void QSimCreateDialog::slotIncidentsRemoveAll()
{
	unsigned int i;
	std::map<QString, std::map<QString, QString> >::iterator iterValues;
	
	for (i = 0; i < m_vecIncidents.size(); i++)
	{
		iterValues = m_mapModels.find(m_vecIncidents[i].first);
		m_mapModels.erase(iterValues);
	}
	m_listIncidents->clear();
	m_vecIncidents.clear();
	m_buttonIncidentRemoveAll->setEnabled(m_listIncidents->count() > 0);
	m_listIncidents->setCurrentItem(-1);
	slotIncidentsListHighlighted(m_listIncidents->currentItem());
	slotIncidentsListSelChanged();
}
//------------------------------added by Cong---------------------------------
...
void QSimCreateDialog::slotOK()
{
	...
	for (i = 0; i < m_vecIncidents.size(); i++)
	{
		iterModel = m_mapModels.find(m_vecIncidents[i].first);
		mapParams = iterModel->second;
		mapParams[PARAM_TYPE] = m_vecIncidents[i].second;
		mapModels.insert(std::pair<QString, std::map<QString, QString> >(m_vecIncidents[i].first, mapParams));
		mapParams.clear();
	}
	//-------------------------added by Cong
...
void QSimCreateDialog::AddExistingModels()
{
	...
	for (i = 0; i < iModelNodes; i++)
	{
		...
		if (pModelNodes[i].pModel != NULL)
		{
			...
			bIncident = pModelNodes[i].pModel->IsModelTypeOf(INCIDENTMODEL_NAME, true); // added by Cong
			...
			{
				m_vecIncidents.push_back(std::pair<QString, QString>(strName, strModelType));
				m_listIncidents->insertItem(QString("[%1] %2").arg(strModelType).arg(strName));
			}
			//---
			else
			{
	...
	//--------
	m_buttonIncidentRemoveAll->setEnabled(m_listIncidents->count() > 0);
	m_listIncidents->setCurrentItem(m_listIncidents->count() > 0 ? 0 : -1);
	slotIncidentsListHighlighted(m_listIncidents->currentItem());
	slotIncidentsListSelChanged();
	//------added by Cong	
...
void QSimCreateDialog::AddTableItem(QTable * pTable, int row, int col, const QString & strValue, const ModelParameter * pParam)
{
	...
	case ModelParameterTypeGPS:
		...
		break;
		//---added by Cong
	case ModelParameterTypeSeve:
		listOptions.push_back("Level 0");
		listOptions.push_back("Level 1");
		listOptions.push_back("Level 2");
		listOptions.push_back("Level 3");
		listOptions.push_back("Level 4");
		listOptions.push_back("Level 5");
		pTable->setItem(row, col, pComboItem = new QComboTableItem(pTable, listOptions, false));
		pComboItem->setCurrentItem(strValue);
		break;
	case ModelParameterTypeRegion: 
		listOptions.push_back("Circle");
		listOptions.push_back("Square");
		listOptions.push_back("Rectangle");
		pTable->setItem(row, col, pComboItem = new QComboTableItem(pTable, listOptions, false));
		pComboItem->setCurrentItem(strValue);
		break;
		//---

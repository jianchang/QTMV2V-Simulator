...
#include "IncidentRegistry.h" //added by Cong
...
CarModel & CarModel::operator =(const CarModel & copy)
{
	...
	m_vitIncidents = copy.m_vitIncidents;
...
int CarModel::ProcessEvent(SimEvent & event)
{
	...
	case EVENT_CARMODEL_UPDATE:
	{
		...
		//---
		// check if any incident is observed
		
		std::map<Coords, IncidentModel *> * pIncidentRegistry;
		std::map<Coords, IncidentModel *>::iterator iterIncident;
		std::pair<map<Coords, IncidentModel *>::iterator, bool> ret;
		std::map<Coords, IncidentModel *>::iterator iter;
		...
		if (pIncidentRegistry->size() != 0)
		{
			...
			{
				Coords incidentLocation = iterIncident->second->GetIncidentLocation();

				if (iterIncident->second->GetRegion() == "Circle")
				{
					if (Distance(incidentLocation, m_ptPosition) * METERSPERMILE <= INCIDENTREGION_CIRCLE_RADIUS && m_iCurrentRecord == iterIncident->second->GetCurrentRecord())
					{
						ret = m_vitIncidents.insert(std::pair<Coords, IncidentModel *>(incidentLocation, iterIncident->second));
						if (ret.second == true)
							sendIncidentMessage(iterIncident->second);
					}
					else
					{
						iter = m_vitIncidents.find(incidentLocation);
						if (iter != m_vitIncidents.end())
							m_vitIncidents.erase(iter);
					}
				}
				/*else if (iterIncident->second->GetRegion() == "Square")
				{
					// to be added
				}
				else if (iterIncident->second->GetRegion() == "Rectangle")
				{
					// to be added
				}*/
	...
	case EVENT_CARMODEL_RXMESSAGEEND:
	{
		...
		if (bValid)
		{
			...
			case ptSafety:
			{
				...
				// received message successfully
				//std::cout << QString("<SafetyPacket from %1 received by %2>  %3").arg(IPAddressToString(pMsg->m_ID.srcID.ipCar)).arg(IPAddressToString(pMsg->m_ipRX)).arg((const char *)pMsg->m_pData).latin1();
...
void CarModel::sendIncidentMessage(IncidentModel * pIncident)
{
	Event1Message msgEvent;
	SafetyPacket msg;
	CreateMessage(&msg);
	QString str1 = "Incident Ahead: ";
	QString str2 = " at ";
	//SafetyPacket::BoundingRegion sBoundingRegion;
	//msg.m_sBoundingRegion = sBoundingRegion;
	msg.m_pData = (unsigned char *)strdup(str1 + pIncident->GetDescription() + str2 + pIncident->GetIncidentAddress());
	msg.m_iDataLength = str1.length() + pIncident->GetDescription().length() + str2.length() + pIncident->GetIncidentAddress().length() + 1;

	g_pSimulator->m_mutexEvent1Log.lock();
	msgEvent.ID = msg.m_ID.srcID;
	msgEvent.tMessage = g_pSimulator->m_tCurrent;
	msgEvent.tLifetime = msg.m_tLifetime = pIncident->GetIncidentDurationTime();
	msgEvent.ptOrigin = m_ptPosition;
	...
	if (m_pCommModel != NULL)
		m_pCommModel->AddMessageToRebroadcastQueue(msg);
}
